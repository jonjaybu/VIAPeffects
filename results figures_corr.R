...
r1 <- ggplot(results1) +
  ...
  geom_line(aes(x=time, y=ci, group=factor(any_tx_x), color=factor(any_tx_x), linetype=factor(any_tx_x)), 
            size=1.2) +
  scale_color_manual(values=c(colors[1], colors[2]), labels=c("Control", "Treatment"), name="") +
  scale_linetype_manual(values = c(lines[1], lines[2]), labels=c("Control", "Treatment"), name="") +   
